...
import TextareaAutosize from 'react-textarea-autosize'; // <-- 1. IMPORTED
...
// --- SelectionPopup for Floating "Ask about this" Button ---
...
const Workspace = () => {
    ...
    const fetchData = async () => {
      try {
        ...
        const [userResponse, pdfResponse] = await Promise.all([
          ...
          axios.get(`http://localhost:3000/api/docs/file/${documentId}`, { headers: { Authorization: `Bearer ${token}` }, responseType: 'blob' })
        ]);
  ...
  // --- Text selection handlers for popup ---
...
const handleGenerateSummary = async () => {
  ...
  try {
    ...
    const response = await axios.post(
      `http://localhost:3000/api/ai/summarize/${documentId}`,
      null, // <-- The request body (can be null or {} if you're not sending data)
      {     // <-- The config object with headers is the THIRD argument
        headers: { 
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await axios.post(
        `http://localhost:3000/api/ai/chat/${documentId}`,
